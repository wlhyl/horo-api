...
fn planet_aspect_ascmc_plant_orb_is_0(aspect_value: u8) {
    for p0_name in [ASC, MC, DSC, IC] {
        ...
        for p1_long in p1_longs {
            ...
            let aspect = asc.has_aspect(&p1, true);
            assert!(aspect.is_some());
            let aspect = aspect.unwrap();
            assert_eq!(
                aspect_value, aspect.aspect_value,
                "{:?}{}度相位{:?}",
                p1.name, aspect_value, asc.name
            );
            assert_eq!(false, aspect.apply, "{:?}与{:?}出相位", p1.name, asc.name);
            assert_eq!(0.0, aspect.d, "{:?}与{:?}出相位0.0度", p1.name, asc.name);
...
// 行星相位四轴，行星容许度不为0，行星顺行，没有跨星座
#[parameterized(aspect_value = {
    0, 60, 90, 120, 180},
)]
fn planet_aspect_ascmc_plant_orb_is_not_0_planet_forward_no_cross_sign(aspect_value: u8) {
    let p0_longs = [
...
// 行星相位四轴，行星容许度不为0，行星逆行，没有跨星座
#[parameterized(aspect_value = {
    0, 60, 90, 120, 180},
)]
fn planet_aspect_ascmc_plant_orb_is_not_0_planet_retrograde_no_cross_sign(aspect_value: u8) {
    let p0_longs = [
        10.0 + f64::from(aspect_value),
        swe_degnorm(360.0 - f64::from(aspect_value) + 10.0),
    ];
...
// 行星相位四轴，行星容许度不为0，行星顺行，跨星座入相位
#[parameterized(aspect_value = {
    0, 60, 90, 120, 180}
)]
fn planet_aspect_ascmc_plant_orb_is_not_0_planet_forward_cross_sign_apply(aspect_value: u8) {
    let p0_longs = [
...
// 行星相位四轴，行星容许度不为0，行星逆行，跨星座入相位
#[parameterized(aspect_value = {
    0, 60, 90, 120, 180}
)]
fn planet_aspect_ascmc_plant_orb_is_not_0_planet_retrograde_cross_sign_apply(aspect_value: u8) {
    let p0_longs = [
...
// 行星相位四轴，行星容许度不为0，行星顺行，跨星座离相位
#[parameterized(aspect_value = {
    0, 60, 90, 120, 180}
)]
fn planet_aspect_ascmc_plant_orb_is_not_0_planet_forward_cross_sign_separated(aspect_value: u8) {
    let p0_longs = [
        28.0 + f64::from(aspect_value),
        swe_degnorm(360.0 - f64::from(aspect_value) + 28.0),
    ];
...
// 行星相位四轴，行星容许度不为0，行星逆行，跨星座离相位
#[parameterized(aspect_value = {
    0, 60, 90, 120, 180}
)]
fn planet_aspect_ascmc_plant_orb_is_not_0_planet_retrograde_cross_sign_separated(aspect_value: u8) {
    let p0_longs = [
        2.0 + f64::from(aspect_value),
        swe_degnorm(360.0 - f64::from(aspect_value) + 2.0),
    ];
