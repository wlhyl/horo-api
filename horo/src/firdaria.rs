...
mod test {
    ...
    use crate::{PlanetConfig, PlanetName, firdaria_process};
    use std::env;
    ...
    fn create_end_date(start_date: HoroDateTime, year_num: u8) -> HoroDateTime {
        HoroDateTime::new(
    ...
    fn test_firdaria_diurnal() {
        ...
        let native_date = HoroDateTime::new(2024, 3, 1, 12, 0, 0, 8.0).unwrap();
        ...
        let firdaria = firdaria_process(native_date, geo, &planets_config, &ephe_path).unwrap();
        ...
        for f in &firdaria {
            assert_eq!(f.sub_period[0].start_date.month, 3);
            assert_eq!(f.sub_period[0].start_date.day, 1);
            assert_eq!(f.sub_period[0].start_date.hour, 12);
            assert_eq!(f.sub_period[0].start_date.minute, 0);
            assert_eq!(f.sub_period[0].start_date.second, 0);
        }
        ...
        for f in firdaria {
            if f.sub_period.len() == 1 {
                ...
                let year_num = f.period.firdaria_year_number().unwrap();
                let start_date = f.sub_period[0].start_date;
                let end_date = create_end_date(start_date, year_num);
                let days = (end_date.jd_utc - start_date.jd_utc) / 7.0;

                // 测试开始时间和行星
                for n in 0u8..7 {
                    let date = start_date.plus_days(days * f64::from(n)).unwrap();
                    let k = SUB_PERIOD_SERIES
                        .iter()
                        .position(|&p| p == f.period)
                        .unwrap();
                    let n_usize: usize = n.into();

                    assert_eq!(
                        f.sub_period[n_usize].period,
                        SUB_PERIOD_SERIES[(k + n_usize) % 7],
                        "第{n}个子周期"
                    );

                    // 会因浮点数，有1秒的误差
                    assert!(
                        (f.sub_period[n_usize].start_date.jd_utc - date.jd_utc).abs()
                            * 24.0
                            * 3600.0
                            < 1.0,
                        "第{n}个子周期"
    ...
    fn test_firdaria_night() {
        ...
        let native_date = HoroDateTime::new(2024, 3, 1, 0, 0, 0, 8.0).unwrap();
        let geo = GeoPosition::new(
            102.0 + 41.0 / 60.0 + 59.0 / 3600.0,
            25.0 + 1.0 / 60.0 + 53.0 / 3600.0,
        )
        ...
        assert_eq!(firdaria[4].period, PlanetName::Sun);
        assert_eq!(firdaria[5].period, PlanetName::Venus);
        assert_eq!(firdaria[6].period, PlanetName::Mercury);
        assert_eq!(firdaria[7].period, PlanetName::NorthNode);
        assert_eq!(firdaria[8].period, PlanetName::SouthNode);
        ...
        assert_eq!(firdaria[0].sub_period[0].start_date.year, 2024);
        assert_eq!(firdaria[1].sub_period[0].start_date.year, 2024 + 9); // 月: 9
        assert_eq!(firdaria[2].sub_period[0].start_date.year, 2024 + 9 + 11); // 土: 11
        assert_eq!(
            firdaria[3].sub_period[0].start_date.year,
            2024 + 9 + 11 + 12 // 木: 12
        );
        ...
        assert_eq!(
            firdaria[5].sub_period[0].start_date.year,
            2024 + 9 + 11 + 12 + 7 + 10 // 日: 10
        );
        assert_eq!(
            firdaria[6].sub_period[0].start_date.year,
            2024 + 9 + 11 + 12 + 7 + 10 + 8 // 金: 8
        );
        assert_eq!(
            firdaria[7].sub_period[0].start_date.year,
            2024 + 9 + 11 + 12 + 7 + 10 + 8 + 13 // 水: 13
        );
        assert_eq!(
            firdaria[8].sub_period[0].start_date.year,
            2024 + 9 + 11 + 12 + 7 + 10 + 8 + 13 + 3 // 北交点: 3
        );

        for f in &firdaria {
            assert_eq!(f.sub_period[0].start_date.month, 3);
            assert_eq!(f.sub_period[0].start_date.day, 1);
            assert_eq!(f.sub_period[0].start_date.hour, 0);
            assert_eq!(f.sub_period[0].start_date.minute, 0);
            assert_eq!(f.sub_period[0].start_date.second, 0);
        }
        ...
        for f in firdaria {
            if f.sub_period.len() == 1 {
                ...
                for n in 0u8..7 {
                    ...
                    // 不直接比较year, month,day,hour,minute,second，
                    // 因为plus_days()是从jd_utc反算回日期，时间会因浮点数，有1秒的误差
                    // 误差小于1秒，即认为相等
                    assert!(
                        (f.sub_period[n_usize].start_date.jd_utc - date.jd_utc).abs()
                            * 24.0
