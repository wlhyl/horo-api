...
export class ImageBasic {
  protected planets: Array<PlanetName> = this.config.horoPlanets;

  constructor(
    // protected http :HttpClient,
    protected config: Horoconfig // protected storage :HorostorageService, // protected platform: Platform,
  ) {}
  ...
  protected drawAspect(
    ...
    for (let i = 0; i < this.planets.length; i++) {
      ...
      let planetString = this.config.planetFontString(planet);
      let planetFontFamily = this.config.planetFontFamily(planet);
    ...
    for (let aspect of aspects) {
      let fontSize = width / col;
      let aspectFontFamily = this.config.aspectFontFamily();

      // 第一格中心的坐标，即太阳与太阳相交的格子
      // x: width / col + width / col / 2
      // y: heigth / row + heigth / row / 2
      let cx = (width / col) * (1.5 + this.planets.indexOf(aspect.p0));
      let cy = (heigth / row) * (1.5 + this.planets.indexOf(aspect.p1));
      let aspectString = this.config.aspectFontString(aspect.aspect_value);
  ...
  protected drawHorosco(
    ...
    this.drawHouse(horosco.houses_cups, canvas, {
      cx: cx,
      cy: cy,
      r0: r0,
      r1: r1,
    ...
    // 画一些注释文本
    this.drawNotes(horosco, canvas, options);
  }
  /**
   * 绘制宫头
   * @param cups 宫头度数
  ...
  private drawHouse(
    cups: Array<number>,
    canvas: Canvas,
    options: { cx: number; cy: number; r0: number; r1: number }
  ) {
    let cx = options.cx;
    let cy = options.cy;
    ...
    for (let i = 0; i < cupsOfImage.length; i++) {
      ...
      // d: 宫位宽度，>=0
      let d = degNorm(
        cupsOfImage[(i + 1) % cupsOfImage.length] - cupsOfImage[i]
      );
      ...
      let zoodiacText = new fabric.Text(
        `${this.config.zodiacFontString(cupsZoodiacLong.zodiac)}`,
        {
          fontSize: (r0 - r1) / 2,
      ...
      let zoodiacLongDText = new fabric.Text(`${cupsZoodiacLongDMS.d};`, {
        fontSize: (r0 - r1) / 4,
        fontFamily: this.config.astrologyFont,
      });
  ...
  private drawPlanets(
    ...
    for (let i = 0; i < p.length; i++) {
      ...
      // 绘制行星名
      let planetString = this.config.planetFontString(planets[i].name);
      let fontSize = 30;
      if (planetString.length > 1) fontSize = 15;
      let planetText = new fabric.Text(planetString, {
        fontSize: fontSize,
        fontFamily: this.config.planetFontFamily(planets[i].name),
      });
      ...
      let zoodiaText = new fabric.Text(
        `${this.config.zodiacFontString(planetLongOnZoodiac.zodiac)}`,
        {
          fontSize: fontSize,
          fontFamily: this.config.astrologyFont,
        }
  ...
  private drawNotes(
    ...
    noteText = new fabric.Text(
      `${this.config.planetFontString(horosco.planetary_day)}`,
      {
        fontSize: fontSize,
        fontFamily: this.config.planetFontFamily(horosco.planetary_day),
        top: noteText.top,
        left: noteText.width,
      }
    );
    ...
    canvas.add(noteText);
    noteText = new fabric.Text(
      `${this.config.planetFontString(horosco.planetary_hours)}`,
      {
        fontSize: fontSize,
        fontFamily: this.config.planetFontFamily(horosco.planetary_hours),
        top: noteText.top,
        left: noteText.width,
      }
    );
