...
export function drawAspect(
  ...
  // 左侧坐标(width / 14, 0 + i*hegith / 14 )，第一列是行星，因此x坐标从第二个格开始，每格宽：width/14，因此，x0=width/14
  ...
  for (let aspect of aspects) {
    ...
    const cy = (heigth / row) * (1.5 + planets.indexOf(aspect.p1));
